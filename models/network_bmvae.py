""" Multi-Varitional AE for Entanglement Representation Learning in video frames
...
import torch
from models.local_components import (
    resnet18_decoder,
    resnet18_encoder,
)
import numpy as np
...
class OutConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, 11, kernel_size=3, stride=1, padding=1, bias=False),
                                    nn.Conv2d(11, 33, kernel_size=3, stride=1, padding=1, bias=False),
                                    nn.Conv2d(33, out_channels, kernel_size=1))
...
class MVAE(pl.LightningModule):
    def __init__(self, enc_out_dim=512, latent_dim=250, input_height=256, batch_size = 32, n_calsses = 66):
        super().__init__()

        self.save_hyperparameters()
        self.n_classes = n_calsses
        # encoder, decoder
        self.encoder = resnet18_encoder(True, False)
        ...
        self.outc = OutConv(3, self.n_classes)
    ...
    def training_step(self, batch, batch_idx):
        ...
        seg_loss = cons_loss + ce_loss

        # total_loss
        total_loss = seg_loss + 5*kld_loss 
    ...
    def validation_step(self, batch, batch_idx):
        ...
        x_hat = self.decoder(z) 
        logits = self.outc(x_hat).float()
        mask_pred = torch.argmax(logits, dim=1)
